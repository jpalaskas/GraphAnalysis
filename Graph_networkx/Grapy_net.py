...
def jac_sim(total_attacks, total_trades, total_messages):
    # best_in_attacks = total_attacks.nlargest(100, 'degree').to_numpy()
    # best_in_trades = total_trades.nlargest(100, 'degree').to_numpy()
    # best_in_messages = total_messages.nlargest(100, 'degree').to_numpy()
    ...
    # b1 = set(best_in_attacks[:, [0]])
    # b2 = set(best_in_trades[:, [0]])
    # b3 = set(best_in_messages[:, [0]])
    ...
    # jac_attacks_messages = jaccard_score(best_in_attacks[:, [0]], best_in_messages[:, [0]], average="macro")
    # jac_attacks_trades = jaccard_score(best_in_attacks[:, [0]], best_in_trades[:, [0]], average="macro")
    # jac_messages_trades = jaccard_score(best_in_trades[:, [0]], best_in_messages[:, [0]], average="macro")
    all_sim = []
...
def read_csv_files():
    ...
    for file in file_names:
        ...
        # formatted_time_stamp = datetime.utcfromtimestamp(df['Timestamp']).strftime('%Y-%m-%d %H:%M:%S')
        # splitted_time_stamp = formatted_time_stamp.split()
        # x = 1

        G = nx.from_pandas_edgelist(df=df, source='id1', target='id2', edge_attr='label')
        pos = nx.spring_layout(G, k=10)
        nx.draw(G, pos, with_labels=True)
        labels = {e: G.edges[e]['label'] for e in G.edges}
        nx.draw_networkx_edge_labels(G, pos, edge_labels=labels)
